...
public class FileChooser extends ListActivity {
	...
	private fr.product.greg.pleiadmini.FileArrayAdapter adapter;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (extras != null) {
			if (extras.getStringArrayList("filterFileExtension") != null) {
				...
				Log.d("AAA", extensions.size()+ "");
				fileFilter = new FileFilter() {
					@Override
					public boolean accept(File pathname) {
						Log.d("AAA", pathname.getName()+ "");
						Log.d("AAA", pathname.getName()+ "");
	...
	private void fill(File f) {
		...
		if (!f.getName().equalsIgnoreCase("sdcard")) {
			...
			//	dir.add(1, new Option("Choisir", f.getPath().toString(), f.getParent(), false, true));
				EspacePerso.setLastClickPath(f.getPath().toString()) ;
